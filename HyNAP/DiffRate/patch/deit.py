...
class DiffRateBlock(Block):
    ...
    """
    def introduce_diffrate(self,patch_number, prune_granularity, merge_granularity, use_adjacent_merge=False):
        self.prune_ddp = DiffRate(patch_number,prune_granularity)
        self.merge_ddp = DiffRate(patch_number,merge_granularity)
        self.use_adjacent_merge = use_adjacent_merge
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_attn, attn, metric = self.attn(self.norm1(x), size, mask=self._diffrate_info["mask"])
        ...
        _, idx = torch.sort(fused_score, descending=True)
...
class DiffRateAttention(Attention):
    ...
    def forward(
        ...
        k_mean = k.mean(1)  # [B, N, C//H]
        return x, attn, k_mean
