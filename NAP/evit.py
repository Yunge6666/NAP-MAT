...
class Attention(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None):
        ...
        if (self.keep_rate < 1 and keep_rate < 1) or tokens is not None:
            ...
            cls_attn = cls_attn.mean(dim=1)      # [B, M]  M = N-1
            token_received_attn = attn[:, :, :, 1:].mean(dim=2)  # [B, H, N-1] average all source token
            Received_attn = token_received_attn.mean(dim=1)  # [B, N-1] average in head dimension

            R = 1 # adjustable
            B, M = Received_attn.shape
            device = Received_attn.device
            d = torch.arange(0, 2*R+1, device=device).float()   # [0 … 2R]
            dist = torch.abs(d - R)                             # [R … 0 … R]
            kernel = 1.0 / (dist + 1.0)                     
            kernel[R] = 0.0                                     # not include itself
            kernel = kernel / kernel.sum()                      # normalize
            kernel = kernel.view(1, 1, 2*R+1)                   # [out_c=1, in_c=1, L]

            attn_pad = F.pad(Received_attn.unsqueeze(1), (R, R), mode='constant', value=0.0)  # [B,1,M+2R]
            neighbor_support = F.conv1d(attn_pad, weight=kernel, padding=0)             # [B,1,M]
            neighbor_support = neighbor_support.squeeze(1)                              # [B,M]

            alpha = 0.1         # neighbor support weight, adjustable
            fused_score = (1 - alpha) * cls_attn + alpha * neighbor_support             # [B,M]

            _, idx = torch.topk(fused_score, left_tokens, dim=1, largest=True, sorted=True)  # [B, left_tokens]
...
class EViT(nn.Module):
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        # apply Hilbert sequence reordering (after positional encoding)
        patch_h = h // self.patch_embed.patch_size[0]
        patch_w = w // self.patch_embed.patch_size[1]
        
        if patch_h * patch_w == (x.shape[1] - self.num_tokens):  # ensure patch number matches
            B, total_tokens, C = x.shape
            
            special_tokens = x[:, :self.num_tokens]  # class token (and distillation token if exists)
            patch_tokens = x[:, self.num_tokens:]    # patch tokens
            
            # rearrange patch tokens to 2D grid: (B, patch_h, patch_w, C)
            patch_tokens_2d = patch_tokens.view(B, patch_h, patch_w, C)
            
            # apply Hilbert sequence reordering: (B, patch_h, patch_w, C) -> (B, N, C)
            patch_tokens_reordered = tensor_to_gilbert_path(patch_tokens_2d, cache=_global_gilbert_cache)
            
            # concatenate special tokens and reordered patch tokens
            x = torch.cat([special_tokens, patch_tokens_reordered], dim=1)
...
@register_model
def vit_base_patch16_mae(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, keep_rate=keep_rate)
    model_kwargs.update(kwargs)
    model = _create_evit('vit_base_patch16_mae', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def vit_large_patch16_mae(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, keep_rate=keep_rate)
    model_kwargs.update(kwargs)
    model = _create_evit('vit_large_patch16_mae', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def vit_huge_patch14_mae(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
        keep_rate[loc] = base_keep_rate
    model_kwargs = dict(patch_size=14, embed_dim=1280, depth=32, num_heads=16, keep_rate=keep_rate)
    model_kwargs.update(kwargs)
    model = _create_evit('vit_huge_patch14_mae', pretrained=pretrained, **model_kwargs)
    return model

# -------------------------------------------------------------
# Some example EViT models
@register_model
def deit_small_patch16_224_shrink_base(pretrained=False, base_keep_rate=0.5, **kwargs):
